...
# I changed this to match exactly what you typed: @RictaTerminalbot
    ...
    while True:
        ...
        try:
            with open(DB_PATH, 'rb') as f:
                await app.bot.send_document(chat_id=ADMIN_ID, document=f, caption=f"📅 Daily Backup: {datetime.now().strftime('%Y-%m-%d')}")
        except: pass
...
async def handle_photo_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Deep Link - No underscores as per your tag @RictaTerminalbot
...
async def expire_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row and row[0]:
        try:
            await context.bot.edit_message_caption(chat_id=CHANNEL_ID, message_id=row[0], caption=f"🏆 <b>Game #{pid}</b>\n━━━━━━━━━━━━━━━\nStatus: <b>EXPIRED</b>", reply_markup=None, parse_mode="HTML")
        except: pass
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.id == ADMIN_ID:
        await update.message.reply_text("⚡ <b>TERMINAL DASHBOARD</b>\n/admin for control.", parse_mode="HTML")
    else:
...
def main():
    ...
    app.add_handler(CommandHandler("online", lambda u,c: u.message.reply_text(f"🛰️ <b>Online Games:</b> " + ", ".join([str(r[0]) for r in run_query("SELECT post_id FROM posts WHERE is_expired = 0", fetch_all=True)]) or "None", parse_mode="HTML")))
    ...
    # Admin
    app.add_handler(CommandHandler("admin", lambda u,c: u.message.reply_text("⚙️ <b>ADMIN</b>\n/list | /audit | /whois ID\n/approve ID | /remove ID\n/online | /report\n/edit ID TEXT | /expire ID | /delete ID\n/backup | /clearstats", parse_mode="HTML") if u.effective_user.id == ADMIN_ID else None))
    ...
    app.add_handler(CommandHandler("backup", lambda u,c: u.message.reply_document(open(DB_PATH, 'rb')) if u.effective_user.id == ADMIN_ID else None))
