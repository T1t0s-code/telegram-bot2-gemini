...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if CHANNEL_ID:
        try:
            ...
            await update.message.reply_text(f"✅ Data #{post_id} broadcasted to Terminal.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not post:
        await query.answer("Data expired or removed.", show_alert=True)
        return
    ...
    if datetime.utcnow() > created_dt + timedelta(minutes=EXPIRY_MINUTES):
        await query.answer("Selection timeline expired.", show_alert=True)
        return

    # Log the claim & Send DM
