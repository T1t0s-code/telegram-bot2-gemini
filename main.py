...
def db_init():
    ...
    # Migration for message tracking
...
async def expiry_monitor(context: ContextTypes.DEFAULT_TYPE):
    """Background task that runs every minute to close old games."""
    expired_posts = run_query(
        "SELECT post_id, channel_msg_id FROM posts WHERE is_expired = 0 AND created_at < ?", 
        ((datetime.now() - timedelta(minutes=EXPIRY_MINUTES)).strftime('%Y-%m-%d %H:%M:%S'),), 
        fetch_all=True
    )
    ...
    for post_id, msg_id in expired_posts:
        ...
        if CHANNEL_ID and msg_id:
            try:
                # Edit channel post to show Expired Status and remove button
                new_text = (
                    f"🏆 **Game #{post_id}**\n"
                    f"━━━━━━━━━━━━━━━\n"
                    f"📍 Status: **EXPIRED**\n"
...
async def handle_photo_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if CHANNEL_ID:
        ...
        try:
            ...
            # Store message ID so we can edit it later when it expires
...
async def audit_partners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Complex query to get Name, Total Unique Games, and IDs of those games
    data = run_query("""
...
def main():
    ...
    # Setup Background Monitor (Runs every 60 seconds)
    job_queue = app.job_queue
    job_queue.run_repeating(expiry_monitor, interval=60, first=10)
    ...
    app.add_handler(CallbackQueryHandler(lambda u, c: button_callback(u, c))) # Linked logic
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    error = await deliver_game(u_id, game_id, context)
